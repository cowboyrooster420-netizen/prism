-- ===============================================
-- HISTORICAL OHLCV DATA COLLECTION SYSTEM
-- ===============================================
-- Schema for storing historical candle data for technical analysis

-- Create historical OHLCV table
CREATE TABLE IF NOT EXISTS token_ohlcv_history (
    ...
    -- Time data
    timestamp_unix BIGINT NOT NULL, -- Unix timestamp for easy conversion
    ...
    number_of_trades INTEGER DEFAULT 0 CHECK (number_of_trades >= 0),
...
-- Composite index for common queries (token + timeframe + time range)
...
CREATE INDEX IF NOT EXISTS idx_ohlcv_recent ON token_ohlcv_history(token_address, timeframe, timestamp_utc DESC) 
...
BEGIN
    ...
    -- If no data exists, return a reasonable starting point (30 days ago)
    IF latest_timestamp IS NULL THEN
        latest_timestamp := NOW() - INTERVAL '30 days';
    END IF;
...
-- Function to clean old OHLCV data (to manage storage)
CREATE OR REPLACE FUNCTION cleanup_old_ohlcv_data(days_to_keep INTEGER DEFAULT 90)
RETURNS INTEGER AS $$
DECLARE
    deleted_count INTEGER;
BEGIN
    DELETE FROM token_ohlcv_history
...
BEGIN
    ...
    FROM token_ohlcv_history h
    LEFT JOIN tokens t ON h.token_address = t.mint_address
    GROUP BY h.token_address, t.symbol
...
-- Create a sample data collection tracking table
...
-- Index for collection status
CREATE INDEX IF NOT EXISTS idx_collection_status_token ON ohlcv_collection_status(token_address, timeframe);
CREATE INDEX IF NOT EXISTS idx_collection_status_active ON ohlcv_collection_status(is_active) WHERE is_active = true;

-- Grant permissions for collection status
GRANT ALL ON ohlcv_collection_status TO service_role;
GRANT SELECT ON ohlcv_collection_status TO authenticated;

-- Show setup results
SELECT 'Historical OHLCV schema created successfully!' as status;
SELECT 'Functions created: get_token_ohlcv_history, get_latest_ohlcv_timestamp, cleanup_old_ohlcv_data, get_ohlcv_data_summary' as functions;