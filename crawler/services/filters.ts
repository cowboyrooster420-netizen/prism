...
export function filterTokens(tokens: Token[]): Token[] {
  ...
  const basicFiltered = tokens.filter(token => {
    ...
    if (token.holder_count && token.holder_count < MIN_HOLDER_COUNT) {
      console.log(`Filtered out low holder count: ${token.name} (${token.holder_count} holders)`);
      return false;
    }
    
    if (token.volume_24h && token.volume_24h < MIN_VOLUME_24H) {
      console.log(`Filtered out low volume: ${token.name} ($${token.volume_24h} volume)`);
      return false;
    }
    
    if (token.market_cap && token.market_cap < MIN_MARKET_CAP) {
  ...
  const scoredTokens: TokenScore[] = basicFiltered.map(token => {
  ...
  const targetCount = Math.min(TARGET_QUALITY_TOKENS, scoredTokens.length);
...
function calculateTokenScore(token: Token): { total: number; reasons: string[] } {
  let score = 0;
