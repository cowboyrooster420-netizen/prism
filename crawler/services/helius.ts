...
export async function fetchRecentTokens(): Promise<Token[]> {
  try {
    ...
    for (const address of EXAMPLE_ADDRESSES) {
      try {
        const res = await fetch(
          `https://api.helius.xyz/v0/addresses/${address}/tokens?api-key=${HELIUS_API_KEY}`
        );
        ...
        const heliusTokens: HeliusToken[] = json.tokens || [];
        ...
        // Rate limiting
        await sleep(HELIUS_RATE_LIMIT_MS);
    ...
    const limitedTokens = allTokens.slice(0, MAX_TOKENS_PER_CRAWL);
...
function convertHeliusToToken(heliusToken: HeliusToken): Token {
  return {
